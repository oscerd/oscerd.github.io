...
This command will run a single container with an Apache Cassandra instance (the Cassandra version is the latest tick-tock release 3.7). We could use a single-node cluster but in this way we won't exploit the Cassandra features. So let's add two other nodes!
